...
// ID of the plug-in
static xguid PluginID = { 0xAF000003, 0x00000000, 0x00000001, 0x0000002D };
...
static PropertyDescriptor luminanceRangeProperty =
{ XVT_RangeF, "Luminance Range", "luminanceRange", "Luminance range to filter.", PropertyFlag_None };
...
{
    &hueRangeProperty, &saturationRangeProperty, &luminanceRangeProperty,
    &fillOutsideProperty, &fillColorProperty
};

// Let the class itself know description of its properties
const PropertyDescriptor** HslColorFilterPlugin::propertiesDescription = (const PropertyDescriptor**) pluginProperties;
...
(
    ...
    "HSL Color Filter",
    "HslColorFilter",
    "Filters colors outside/inside of the specified HSL color range.",

    /* Long description */
    "The plug-in filters out pixels (sets them to the specified <b>fill color</b>) which values are outside or "
    "inside of the specified HSL color range. When filling outside mode is used, the filter will keep all "
    "pixels which values are in the specified Hue, Saturation and Luminance ranges. Other pixels, "
...
static void PluginInitializer( )
{
    ...
    // Luminance Range property
    luminanceRangeProperty.DefaultValue.type = XVT_RangeF;
    luminanceRangeProperty.DefaultValue.value.frangeVal.min = 0.0f;
    luminanceRangeProperty.DefaultValue.value.frangeVal.max = 1.0f;

    luminanceRangeProperty.MinValue.type = XVT_R4;
    luminanceRangeProperty.MinValue.value.fVal = 0.0f;

    luminanceRangeProperty.MaxValue.type = XVT_R4;
    luminanceRangeProperty.MaxValue.value.fVal = 1.0f;
