...
const XPixelFormat BrightnessCorrectionPlugin::supportedFormats[] =
{
    XPixelFormatGrayscale8, XPixelFormatRGB24, XPixelFormatRGBA32
};

BrightnessCorrectionPlugin::BrightnessCorrectionPlugin( ) :
    factor( 0.2f ), processRed( true ), processGreen( true ), processBlue( true ), needUpdate( true )
{
    CalculateLinearMap( identityMap, 0, 255, 0, 255 );
}

void BrightnessCorrectionPlugin::Dispose( )
{
    delete this;
}

// The plug-in can process image in-place without creating new image as a result
bool BrightnessCorrectionPlugin::CanProcessInPlace( )
{
    return true;
}

// Provide supported pixel formats
XErrorCode BrightnessCorrectionPlugin::GetPixelFormatTranslations( XPixelFormat* inputFormats, XPixelFormat* outputFormats, int32_t* count )
{
...
XErrorCode BrightnessCorrectionPlugin::ProcessImage( const ximage* src, ximage** dst )
{
...
XErrorCode BrightnessCorrectionPlugin::ProcessImageInPlace( ximage* src )
{
    ...
    if ( needUpdate )
    {
        ...
        float gamma             = ( inverseGammaFilter ) ? factor + 1 : -factor + 1;

        ret = CalculateGammaCorrectionMap( gammaMap, gamma, inverseGammaFilter );
    }
...
XErrorCode BrightnessCorrectionPlugin::GetProperty( int32_t id, xvariant* value ) const
{
    XErrorCode ret = SuccessCode;
...
XErrorCode BrightnessCorrectionPlugin::SetProperty( int32_t id, const xvariant* value )
{
    XErrorCode ret = SuccessCode;
