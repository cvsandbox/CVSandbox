...
const XPixelFormat ErosionPlugin::supportedFormats[] =
{
    XPixelFormatGrayscale8, XPixelFormatRGB24, XPixelFormatRGBA32
};

ErosionPlugin::ErosionPlugin( ) :
...
void ErosionPlugin::Dispose( )
{
    delete this;
}

// The plug-in can not process image in-place
bool ErosionPlugin::CanProcessInPlace( )
{
    return false;
}

// Provide supported pixel formats
XErrorCode ErosionPlugin::GetPixelFormatTranslations( XPixelFormat* inputFormats, XPixelFormat* outputFormats, int32_t* count )
{
...
XErrorCode ErosionPlugin::ProcessImage( const ximage* src, ximage** dst )
{
    ...
    else
    {
        ...
        if ( ret == SuccessCode )
        {
            ...
            if ( ret == SuccessCode )
            {
                ...
                ret = Erosion( src, *dst, structuringElement, seSize );
            }
...
XErrorCode ErosionPlugin::ProcessImageInPlace( ximage* src )
{
...
XErrorCode ErosionPlugin::GetProperty( int32_t id, xvariant* value ) const
{
    XErrorCode ret = SuccessCode;
...
XErrorCode ErosionPlugin::SetProperty( int32_t id, const xvariant* value )
{
    XErrorCode ret = SuccessCode;
