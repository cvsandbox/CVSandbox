...
GenerateFuzzyBorderTexturePlugin::GenerateFuzzyBorderTexturePlugin( ) :
    width( 640 ), height( 480 ), borderWidth( 16 ), gradientWidth( 12 ), waviness( 5 ), randValue( (uint16_t) ( rand( ) % 10000 ) )
{
}

void GenerateFuzzyBorderTexturePlugin::Dispose( )
{
    delete this;
}

// Generate the texture's image
XErrorCode GenerateFuzzyBorderTexturePlugin::GenerateImage( ximage** dst )
{
    ...
    else
    {
        ...
        if ( ret == SuccessCode )
        {
            ret = GenerateFuzzyBorderTexture( *dst, randValue, borderWidth, gradientWidth, waviness );
...
XErrorCode GenerateFuzzyBorderTexturePlugin::GetProperty( int32_t id, xvariant* value ) const
{
    XErrorCode ret = SuccessCode;
...
XErrorCode GenerateFuzzyBorderTexturePlugin::SetProperty( int32_t id, const xvariant* value )
{
    XErrorCode ret = SuccessCode;
