 ...
 * MIN value from the corresponding 3x3 square
...
// forward declaration ----
static void Erosion3x3_8bitPerPlane( const ximage* src, ximage* dst );
// ------------------------
...
XErrorCode Erosion3x3( const ximage* src, ximage* dst )
{
    ...
    {
        Erosion3x3_8bitPerPlane( src, dst );
    }
    else
    {
...
// 3x3 erosion filter for images with 8bpp per plane (8bpp grayscale and 24/32 color)
void Erosion3x3_8bitPerPlane( const ximage* src, ximage* dst )
{
    int width     = src->width;
...

    {
        ...
        for ( i = 0; i < pixelSize; i++ )
        {
            ...
            min1 = XMIN3( srcPtr[pixelSize], srcPtr[srcStride], srcPtr[srcStridePps] );
            *dstPtr = XMIN( *srcPtr, min1 );
            ...
            for ( x = 1; x < widthM1; x++, srcPtr += pixelSize, dstPtr += pixelSize )
            {
                ...
                *dstPtr = XMIN( min1, min2 );
            }

            // last pixel of the first row
            min1 = XMIN3( srcPtr[mPixelSize], srcPtr[srcStride], srcPtr[srcStrideMps] );
            *dstPtr = XMIN( *srcPtr, min1 );
            ...
            min1 = XMIN3( srcPtr[pixelSize], srcPtr[mSrcStride], srcPtr[mSrcStridePps] );
            *dstPtr = XMIN( *srcPtr, min1);
            ...
            min1 = XMIN3( srcPtr[mPixelSize], srcPtr[mSrcStride], srcPtr[mSrcStrideMps] );
            *dstPtr = XMIN( *srcPtr, min1 );
        }
    }
}
